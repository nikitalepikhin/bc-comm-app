...
export default function Votes(props: Props) {
  ...
  return (
    ...
    >
      <IconButton
        disabled={!verified || (role !== "STUDENT" && role !== "TEACHER")}
        onClick={() => sendVote(currentVote === 1 ? 0 : 1)}
        className={classNames(
          "rounded-md p-1",
          {
            "bg-slate-50 hover:bg-slate-100 dark:bg-slate-900 hover:dark:bg-slate-700 disabled:hover:bg-slate-50 dark:disabled:hover:bg-slate-900":
              currentVote !== 1,
          },
      ...
      >
        <ArrowUpIcon
          className={classNames(
            "h-5 w-5 font-extrabold",
            {
              "text-blue-600 dark:text-blue-300": currentVote === 1,
            },
            { "text-slate-300 dark:text-slate-700": !verified }
          )}
        />
      </IconButton>
      <div>{up - down}</div>
