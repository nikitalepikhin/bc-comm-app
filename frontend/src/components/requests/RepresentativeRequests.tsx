...
export default function RepresentativeRequests() {
  const { data, isFetching, isError, refetch, isLoading } = useGetRepresentativeVerificationRequestsQuery();
  ...
  if (isError) {
    return (
      <Alert show={isError} fullWidth severity="error" onClose={refetch}>
        Error loading representative verification requests. Reload or try again later.
      </Alert>
    );
  }
  ...
  return (
    <div className="w-full flex flex-col justify-start gap-2">
      ...
      {data &&
        data.requests.map((request) => <Request key={request.user.uuid} request={request} type="REPRESENTATIVE" />)}
