...
import { enhancedApi, useGetSchoolsQuery } from "../../app/enhancedApi";
...
const selectStyles = "py-0.5 border border-gray-500 rounded-md ml-2";

const SchoolsManagementPage: React.FC = () => {
  const dispatch = useDispatch();
  const { data: queryData, isFetching } = useGetSchoolsQuery(initialValues);
  const [data, setData] = useState<GetSchoolsResponseDto | undefined>(undefined);

  const handleRefetch = useCallback(async ({ page, count }: SchoolsPagingFormValues) => {
    const result = await dispatch(enhancedApi.endpoints.getSchools.initiate({ page, count }, { forceRefetch: true }));
    // @ts-ignore
    setData(result.data);
  }, []);
